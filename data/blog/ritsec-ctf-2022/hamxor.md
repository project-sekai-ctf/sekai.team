...
Throwing resulting `s` to CyberChef, we did not find anything meaningful after all sorts of conversions. Author hinted here that we are missing one step of the hamming process - @blueset suggested that we need to do a fourth step after XOR-ing all blocks:
...
It still does not make much sense. But I have no idea what I missed, so I went to sleep. The second day, I was told that the problem might be due to endianess of bytes, and I quickly realized what was wrong. For the data we received, our endianess when changing to binary is incorrect - currently we did this:
...
Special thanks to **@blueset** for some ideas and help through solving the challenge.
